...
NB:This program uses a combination of in-memory data structures (Python lists and dictionaries) and saved/loaded JSON files for the database.
"""
    )
    ...
    databases_in_folder = [file_name[:-5] for file_name in os.listdir("./database_files")]
...
""").lower()
    ...
    if len(book_database) > 0:
        ...
        if os.path.exists(f"./database_files/{database_name}.JSON"):
            os.remove(f"./database_files/{database_name}.JSON")
